...
export default function Expertises() {
    ...
    return (
        <section className="bg-black text-[#e8e8e2] w-full px-6 py-24 space-y-32" id="expertises">
            <motion.h2
            ...
            {expertises.map((expertise, index) => (
                <motion.div
                    ...
                    className="relative grid md:grid-cols-[1fr_3fr] gap-12 items-start max-w-6xl mx-auto"
                    initial={{ opacity: 0, y: 40 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: false, amount: 0.4 }}
                    transition={{ duration: 0.6, delay: 0.1 * index }}
                >
                    <div className="text-[6vw] font-bold text-[#444] md:text-right text-left">({expertise.id})</div>

                    <div className="space-y-6">
                        <h3 className="text-3xl md:text-4xl font-semibold">{expertise.title}</h3>
                        <p className="text-[#bbb] text-lg leading-relaxed">{expertise.description}</p>
                        <ul className="space-y-3 border-t border-[#333] pt-4">
                            {expertise.bullets.map((bullet, i) => (
                                <li key={i} className="flex gap-3 items-start">
                                    ...
                                    <span className="font-medium text-[#f0f0f0]">{bullet}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
